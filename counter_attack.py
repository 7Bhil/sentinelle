...
class CounterAttack:
    ...
    def flood_fake_responses(self, target_ip):
        print(f"[ COUNTER] Inondation de fausses réponses TCP pour {target_ip}...")
        for _ in range(10):
    ...
    def send_fake_dns_reply(self, target_ip, query_domain):
        print(f"[ COUNTER] Envoi d'une fausse réponse DNS à {target_ip} pour {query_domain}")
        fake_ip = "10.0.0.66" 
    ...
    def fake_network_topology(self, target_ip):
        print(f"[ COUNTER] Génération d'une fausse topologie réseau pour {target_ip}")
    ...
    def corrupt_scan_data(self, target_ip):
        print(f"[ COUNTER] Corruption des outils de scan de {target_ip}")
